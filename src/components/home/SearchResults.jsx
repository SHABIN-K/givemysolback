  ...
  if (!searchResults || (searchResults.totalAccounts === 0 && searchResults.solBalance === 0)) {
    return (
      <div className="max-w-2xl mx-auto mt-16">
        <div className="bg-gray-800/50 backdrop-blur-xl rounded-2xl border border-gray-700/50 p-8 pt-12 text-center">
          <h3 className="text-2xl font-bold text-white mb-4">No assets showing right now</h3>
  ...
  return (
    <div className="max-w-4xl mx-auto mt-16">
      <div className="text-center mb-8">
        ...
        <h2 className="text-3xl font-bold text-white mb-2">What You Own, All Here 🎉</h2>
        <p className="text-gray-400">An instant view of everything that’s yours, together in one overview.</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
        {stats.map((data, idx) => (
          <div key={idx} className="bg-gray-800/50 backdrop-blur-xl rounded-2xl border border-gray-700/50 p-6">
            <div className="text-center">
              <div className={`text-3xl font-bold ${data?.color}`}>{data?.value}</div>
              <div className="text-sm text-gray-400 mt-1">{data?.label}</div>
            </div>
          </div>
        ))}
      </div>
